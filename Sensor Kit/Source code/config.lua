...
function M:init()
	...
	self.configs = {
		...
		["sta_ssid"] = "",
		["sta_pwd"] = "",
		["box_serial"] = "150904-163806-000002",
...
function M:getAPI(isPost)
	if isPost then
		return self.configs["post_api"]
	else
		return self.configs["get_api"] 
	end
end

function M:setAPI(isPost, value)
	if isPost then
		self.configs["post_api"] = value
	else
		self.configs["get_api"] = value
	end
end
